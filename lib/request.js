...
Request.prototype.performRequest = function performRequest(cachedResponse) {
  ...
  if (process.env.HEROKU_HTTP_PROXY_HOST) {
    ...
    console.log('unsecure request debug', this.request);
    req = http.request(requestOptions, this.handleResponse.bind(this));
  } else {
    console.log('secure request debug', this.request);
...
Request.prototype.getRequestOptions = function getRequestOptions(defaultOptions) {
  ...
  if (process.env.HEROKU_HTTP_PROXY_HOST) {
    requestOptions = {
      agent: new http.Agent({ rejectUnauthorized: false, maxSockets: Number(process.env.HEROKU_CLIENT_MAX_SOCKETS) || 5000 }),
    ...
    requestOptions = {
      agent: new https.Agent({rejectUnauthorized: false, maxSockets: Number(process.env.HEROKU_CLIENT_MAX_SOCKETS) || 5000 }),
      host : this.host,
      port : 443,
