...
class Request {
  constructor (options) {
    ...
    if (process.env.HEROKU_HTTP_PROXY_HOST) {
      ...
      if (this.secure) {
        ...
        this.agent = new https.Agent({rejectUnauthorized: false, maxSockets: Number(process.env.HEROKU_CLIENT_MAX_SOCKETS) || 5000 })
      } else {
        var http = require('http')
        this.agent = new http.Agent({rejectUnauthorized: false, maxSockets: Number(process.env.HEROKU_CLIENT_MAX_SOCKETS) || 5000 })
      }
    }
  }
  ...
  request () {
    ...
    console.log('latest req debug', requestOptions);
    let req
    if (this.secure) {
